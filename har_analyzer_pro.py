#!/usr/bin/env python3
"""
HAR Analyzer Pro v3 — Enterprise-grade HAR analysis tool.

Uso:
  python3 har_analyzer_pro.py                           # auto-discovery nella dir corrente
  python3 har_analyzer_pro.py file1.har file2.har       # file specifici
  python3 har_analyzer_pro.py ./cartella/                # tutti i .har nella cartella
  Drag & Drop: trascina file .har sull'eseguibile       # modalita' desktop

Lo script:
  1. Carica tutti i file HAR forniti (o trovati, o trascinati)
  2. Assegna un Health Score a ciascuno (HEALTHY / DEGRADED / BROKEN)
  3. Se esistono sia HAR sani che malati -> analisi differenziale automatica
  4. Altrimenti -> analisi standalone degli HAR problematici
  5. Esegue il Dynamic Rule Engine contro la Knowledge Base (har_known_issues.json)
  6. Analisi di sicurezza (PII, security headers, cookie flags)
  7. Analisi performance (percentili TTFB, compressione, immagini)
  8. Per ogni anomalia stampa diagnosi + soluzione concreta
  9. Genera automaticamente un report HTML enterprise e lo apre nel browser

Report HTML:
  A. Executive Summary — linguaggio naturale per non-esperti, con azioni correttive
  B. Score Card — gauge visivo con codice colore Verde/Giallo/Rosso
  C. Technical Deep Dive — evidenze tecniche, waterfall, tabelle, accordion

PyInstaller:
  Pronto per la compilazione con: pyinstaller --onefile --add-data "har_known_issues.json:." har_analyzer_pro.py
"""
