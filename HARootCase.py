...
class AnalysisResult:
    ...
    root_cause_text: str = ""
    local_ai: Optional[dict] = None  # Optional Local AI RCA review (see --local-ai)
...
def print_performance(stats: PerformanceStats, label: str):
    ...
    if stats.oversized_images:
        ...
        print("    Suggerimento: convertire in WebP/AVIF per ridurre ~30% delle dimensioni.")
...
class HtmlReportGenerator:
    ...
    def _build_html(self, result: AnalysisResult) -> str:
        ...
        # ── AI Local RCA Review (optional, only when --local-ai produced a result) ──
        if getattr(result, "local_ai", None) is not None:
            parts.append('<h3 class="section-title">AI Local RCA Review</h3>')
            parts.append(render_local_ai_html_section(result.local_ai))
...
{reasons_html}
    ...
    def _render_waterfall_svg(self, har: ParsedHar) -> str:
        ...
        lines.append('<text x="4" y="14" font-weight="bold"># St URL</text>')
    ...
    def _render_footer(self) -> str:
        ...
        return f'<div class="footer">Generato da HARootCase v{__version__} &mdash; {now}</div>'
...
# ═══════════════════════════════════════════════════════════════════════════════
#  LOCAL AI HAR RCA REVIEW (optional, offline, Ollama-compatible)
# ═══════════════════════════════════════════════════════════════════════════════
#
# Layer opzionale che usa un LLM locale (Ollama API) come revisore/analista sopra
# i risultati GIA' prodotti dal motore deterministico di HARootCase. Vincoli:
#   - nessuna chiamata cloud;
#   - nessuna nuova dipendenza obbligatoria (solo stdlib: urllib);
#   - il report HTML viene sempre generato anche se l'AI fallisce;
#   - dati sensibili (token, cookie, Authorization, API key, JWT, email, query
#     string sensibili, numeri carta) vengono sanitizzati prima dell'invio.

LOCAL_AI_DEFAULT_ENDPOINT = "http://localhost:11434/api/generate"
LOCAL_AI_DEFAULT_MODEL = "llama3.1:8b"
LOCAL_AI_DEFAULT_TIMEOUT = 300
LOCAL_AI_DEFAULT_MAX_ENTRIES = 40
...
_SENSITIVE_QUERY_KEYS = {
    ...
    "pwd", "sig", "signature", "session", "sessionid", "sid", "code", "jwt",
    "bearer", "x-api-key",
}

_ANSI_RE = re.compile(r"\033\[[0-9;]*m")
# Sensitive `key=value` pairs embedded anywhere in free text (e.g. URLs inside
# root_cause_text or finding details), where the value may be short.
_QUERY_PARAM_RE = re.compile(
    r"(?i)\b(" + "|".join(sorted(_SENSITIVE_QUERY_KEYS, key=len, reverse=True))
    + r")=([^&\s\"'<>]+)")
_JWT_RE = re.compile(r"eyJ[A-Za-z0-9_-]{5,}\.[A-Za-z0-9_-]{5,}\.[A-Za-z0-9_-]{5,}")
_AUTHHEADER_RE = re.compile(
    r"(?i)\b(authorization|proxy-authorization|cookie|set-cookie|x-api-key|api-key|"
    r"x-auth-token|x-csrf-token)\b\s*[:=]\s*[^\n;,]+")
...
def redact_sensitive_text(text: Any) -> str:
    """Redact tokens, cookies, Authorization, API keys, JWT, email, card numbers."""
    if text is None:
        return ""
    s = str(text)
    s = _JWT_RE.sub(_REDACTED, s)
    s = _QUERY_PARAM_RE.sub(lambda m: m.group(1) + "=" + _REDACTED, s)
    s = _AUTHHEADER_RE.sub(lambda m: m.group(1) + ": " + _REDACTED, s)
    s = _BEARER_RE.sub(lambda m: m.group(1) + " " + _REDACTED, s)
    s = _EMAIL_RE.sub(_REDACTED, s)
    s = _CARD_RE.sub(_REDACTED, s)
    s = _LONGTOKEN_RE.sub(_REDACTED, s)
    return s


def _redact_url(url: str) -> str:
    """Redact sensitive query-string parameters while keeping the URL readable."""
    try:
    ...
    for kv in parsed.query.split("&"):
        if "=" in kv:
            ...
            pairs.append(kv)
    try:
        rebuilt = parsed._replace(query="&".join(pairs)).geturl()
    except Exception:
        rebuilt = url
    return redact_sensitive_text(rebuilt)


def sanitize_ai_value(value: Any) -> Any:
    """Recursively sanitize any value (dict/list/str) before sending to the model."""
...
def is_local_endpoint(url: str) -> bool:
    """True only for localhost / 127.0.0.1 / ::1 / RFC1918 private addresses."""
    try:
...
def _entry_ttfb(entry) -> float:
    return entry.timings.wait if getattr(entry, "timings", None) else 0.0


def _summarize_entry(entry, include_raw: bool) -> dict:
    d = {
        "url": _redact_url(entry.url),
        "method": entry.method,
        "status": entry.status,
        "resource_type": _enum_value(entry.resource_type),
        "mime_type": entry.mime_type,
        "domain": entry.domain,
        "time_total_ms": round(entry.time_total, 1),
        "ttfb_ms": round(_entry_ttfb(entry), 1),
        "transfer_size": entry.transfer_size,
        "response_error": redact_sensitive_text(entry.response_error) if entry.response_error else "",
    }
    if include_raw:
        d["response_body_snippet"] = redact_sensitive_text((entry.response_body_snippet or "")[:300])
        d["request_body_snippet"] = redact_sensitive_text((entry.request_body_snippet or "")[:300])
    return d


def _fmt_finding(f) -> dict:
    return {
        "severity": _enum_value(f.severity),
        "category": _enum_value(f.category),
        "type": f.finding_type,
        "title": redact_sensitive_text(f.title),
        "detail": redact_sensitive_text(f.detail),
    }


def build_local_ai_har_context(result: AnalysisResult, max_entries: int = LOCAL_AI_DEFAULT_MAX_ENTRIES,
                               include_raw: bool = False) -> dict:
    """Build a sanitized, compact context dict from already-computed results."""
    n_files = max(1, len(result.parsed_hars))
    per_file_limit = max(1, max_entries // n_files)
    ...
    for har in result.parsed_hars:
        hs = result.health_scores.get(har.file_name)
        entries = har.entries or []
        failed = [e for e in entries if e.status >= 400 or e.response_error]
        failed_sorted = sorted(failed, key=lambda e: e.status, reverse=True)
        slow_sorted = sorted([e for e in entries if e.time_total > 0],
                             key=lambda e: e.time_total, reverse=True)
        critical = [e for e in entries if e.resource_type in (
            ResourceType.DOCUMENT, ResourceType.API, ResourceType.SPA_BUNDLE,
            ResourceType.SPA_CHUNK, ResourceType.STYLESHEET, ResourceType.SCRIPT)]
        all_domains.update(e.domain for e in entries if e.domain)
        all_protocols.update(har.protocols or set())
        files.append({
            "file_name": har.file_name,
            "primary_domain": har.primary_domain,
            "health_score": hs.score if hs else None,
            "health_label": _enum_value(hs.label) if hs else None,
            "health_reasons": [redact_sensitive_text(r) for r in (hs.reasons if hs else [])],
            "entry_count": len(entries),
            "failed_count": len(failed),
            "top_failed_entries": [_summarize_entry(e, include_raw) for e in failed_sorted[:per_file_limit]],
            "top_slow_entries": [_summarize_entry(e, include_raw) for e in slow_sorted[:per_file_limit]],
            "critical_resources": [_summarize_entry(e, include_raw) for e in critical[:per_file_limit]],
        })

    performance = {}
    for fname, st in result.performance.items():
        performance[fname] = {
            "ttfb_p50": round(st.ttfb_p50, 1), "ttfb_p90": round(st.ttfb_p90, 1),
            "ttfb_p99": round(st.ttfb_p99, 1), "download_p50": round(st.download_p50, 1),
            "download_p90": round(st.download_p90, 1), "download_p99": round(st.download_p99, 1),
            "total_transfer_bytes": st.total_transfer_bytes,
            "uncompressed_entries": len(st.uncompressed_entries),
            "oversized_images": len(st.oversized_images),
        }

    security = {}
    for fname, rep in result.security.items():
        security[fname] = {
            # Only counts/messages for PII (never the raw PII values).
            "pii_findings_count": len(rep.pii_findings),
            "header_findings": [redact_sensitive_text(getattr(f, "detail", str(f)))
                                for f in rep.header_findings][:20],
            "cookie_findings": [redact_sensitive_text(getattr(f, "detail", str(f)))
                                for f in rep.cookie_findings][:20],
        }

    differential_extras = {}
    if result.mode == AnalysisMode.DIFFERENTIAL:
        healthy = [h for h in result.parsed_hars
                   if result.health_scores[h.file_name].label == HealthLabel.HEALTHY]
        problematic = [h for h in result.parsed_hars
                       if result.health_scores[h.file_name].label != HealthLabel.HEALTHY]
        if healthy and problematic:
            ref, ko = healthy[0], problematic[0]
            ref_map = {e.entry_key: e for e in ref.entries}
            ko_map = {e.entry_key: e for e in ko.entries}
            missing = [k for k in ref_map if k not in ko_map][:max_entries]
            status_regr, ttfb_regr = [], []
            for k, e_ref in ref_map.items():
                e_ko = ko_map.get(k)
                if not e_ko:
                    continue
                if e_ref.status < 400 <= e_ko.status:
                    status_regr.append({"entry": redact_sensitive_text(k),
                                        "ok_status": e_ref.status, "ko_status": e_ko.status})
                t_ref, t_ko = _entry_ttfb(e_ref), _entry_ttfb(e_ko)
                if t_ko - t_ref > 200 and t_ko > t_ref * 1.5:
                    ttfb_regr.append({"entry": redact_sensitive_text(k),
                                     "ok_ttfb_ms": round(t_ref, 1), "ko_ttfb_ms": round(t_ko, 1)})
            differential_extras = {
                "reference_file": ref.file_name,
                "ko_file": ko.file_name,
                "missing_resources_in_ko": [redact_sensitive_text(m) for m in missing],
                "status_code_regressions": status_regr[:max_entries],
                "ttfb_regressions": ttfb_regr[:max_entries],
            }

    context = {
        "harootcase_version": __version__,
        "analysis_mode": result.mode.value,
        "files_analyzed": [h.file_name for h in result.parsed_hars],
        "files": files,
        "root_cause_text": redact_sensitive_text(_strip_ansi(result.root_cause_text)),
        "red_flags": [_fmt_finding(f) for f in result.red_flags][:max_entries],
        "differential_findings": [_fmt_finding(f) for f in result.differential_findings][:max_entries],
        "differential_extras": differential_extras,
        "kb_matches": [{
            "pattern_id": m.pattern_id, "pattern_name": m.pattern_name,
            "severity": m.severity, "confidence": m.confidence,
            "category": m.category, "diagnosis": redact_sensitive_text(m.diagnosis),
        } for m in result.kb_matches][:max_entries],
        "performance": performance,
        "security": security,
        "domains": sorted(all_domains)[:50],
        "protocols": sorted(str(p) for p in all_protocols),
        "include_raw_snippets": include_raw,
    }
    # Defensive second pass: guarantee nothing sensitive slipped through.
    return sanitize_ai_value(context)


_LOCAL_AI_SCHEMA_HINT = """{
  "root_cause_probabile": "...",
  "confidenza": "ALTA|MEDIA|BASSA",
  "motivazione": "...",
  "modalita_analisi": "ALL_HEALTHY|STANDALONE|DIFFERENTIAL",
  "cause_alternative": ["..."],
  "evidenze_forti": ["..."],
  "evidenze_deboli": ["..."],
  "evidenze_mancanti": ["..."],
  "contraddizioni_o_ambiguita": ["..."],
  "impatto_utente": "...",
  "rischio_falso_positivo": "BASSO|MEDIO|ALTO",
  "azioni_immediate": ["..."],
  "azioni_breve_termine": ["..."],
  "azioni_preventive": ["..."],
  "priorita_remediation": [
    {"priorita": 1, "azione": "...", "impatto": "ALTO|MEDIO|BASSO",
     "complessita": "ALTA|MEDIA|BASSA", "motivazione": "..."}
  ],
  "executive_summary": "...",
  "note_sicurezza_privacy": ["..."]
}"""


def build_local_ai_prompt(context: dict) -> str:
    return (
        "Sei un Senior SRE / Web Performance / Security Engineer.\n"
        "Devi analizzare risultati HAR gia' elaborati da HARootCase.\n\n"
        "Regole:\n"
        "- Usa SOLO i dati forniti.\n"
        "- Non inventare endpoint, domini, status code, header, cookie, metriche, "
        "file o timestamp.\n"
        "- Se la root cause e' incerta, dichiaralo.\n"
        "- Se HARootCase propone una root cause debole, spiegalo.\n"
        "- Distingui tra evidenze forti, evidenze deboli e dati mancanti.\n"
        "- In modalita' DIFFERENTIAL, dai priorita' alle differenze OK vs KO.\n"
        "- In modalita' STANDALONE, dai priorita' a errori HTTP, risorse critiche, "
        "API fallite, TTFB, timeout e security findings.\n"
        "- In modalita' ALL_HEALTHY, evidenzia rischi residui e ottimizzazioni, "
        "senza creare falsi allarmi.\n"
        "- Rispondi esclusivamente con JSON valido secondo lo schema richiesto.\n\n"
        "Schema JSON richiesto:\n" + _LOCAL_AI_SCHEMA_HINT + "\n\n"
        "Dati HARootCase (JSON):\n"
        + json.dumps(context, ensure_ascii=False, indent=2) + "\n"
    )


def _parse_model_json(text: str) -> dict:
    """Parse a JSON object from a model reply, salvaging a truncated trailing object."""
    try:
        return json.loads(text)
    except Exception:
        pass
    # Salvage: try the largest balanced-brace prefix (handles truncated output).
    start = text.find("{")
    if start == -1:
    ...
    raise ValueError("JSON del modello incompleto o non valido")


def call_local_ai_ollama(endpoint: str, model: str, prompt: str,
                         timeout: int = LOCAL_AI_DEFAULT_TIMEOUT) -> dict:
    """POST to an Ollama-compatible /api/generate endpoint and parse the JSON reply."""
    payload = json.dumps({
        "model": model,
        "prompt": prompt,
        "stream": False,
        "format": "json",
        # num_ctx larger than the default so the (potentially long) prompt fits;
        # num_predict so the JSON answer is not truncated mid-object.
    ...
    if not inner:
        raise RuntimeError("campo 'response' mancante nella risposta del modello")
    return _parse_model_json(inner)


def validate_local_ai_har_result(data: dict) -> dict:
    """Validate/normalize the model output against the expected schema."""
    if not isinstance(data, dict):
        raise ValueError("la risposta AI non e' un oggetto JSON")
    str_keys = ["root_cause_probabile", "confidenza", "motivazione", "modalita_analisi",
                "impatto_utente", "rischio_falso_positivo", "executive_summary"]
    list_keys = ["cause_alternative", "evidenze_forti", "evidenze_deboli",
                 "evidenze_mancanti", "contraddizioni_o_ambiguita", "azioni_immediate",
                 "azioni_breve_termine", "azioni_preventive", "note_sicurezza_privacy"]
    out = {}
    for k in str_keys:
    ...
    prio = data.get("priorita_remediation", [])
    clean_prio = []
    if isinstance(prio, list):
        for item in prio:
            if isinstance(item, dict):
                clean_prio.append({
                    "priorita": item.get("priorita", 0),
                    "azione": str(item.get("azione", "") or ""),
                    "impatto": str(item.get("impatto", "") or ""),
                    "complessita": str(item.get("complessita", "") or ""),
                    "motivazione": str(item.get("motivazione", "") or ""),
                })
    out["priorita_remediation"] = clean_prio
    # Sanitize the model's own output defensively before persisting/rendering.
    return sanitize_ai_value(out)


def write_local_ai_json(ai_result: dict, output_path: str) -> None:
    with open(output_path, "w", encoding="utf-8") as fh:
        json.dump(ai_result, fh, ensure_ascii=False, indent=2)


def resolve_local_ai_output(args, har_files: list, html_path) -> str:
    """Resolve the AI JSON output path (honoring --local-ai-output / auto)."""
    chosen = getattr(args, "local_ai_output", "auto")
    if chosen and chosen != "auto":
        return chosen
    if html_path:
        return os.path.splitext(html_path)[0] + "_local_ai.json"
    base_dir = os.path.dirname(har_files[0]) if har_files else _get_default_output_dir()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return os.path.join(base_dir, "HARootCase_%s_local_ai.json" % timestamp)


def run_local_ai_review(result: AnalysisResult, args) -> dict:
    """Orchestrate the local AI review. Never raises: returns a dict either way."""
    meta = {
        "endpoint": args.local_ai_endpoint,
        "model": args.local_ai_model,
        "mode": result.mode.value,
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "strict_local": bool(args.local_ai_strict_local),
        "raw": bool(args.local_ai_raw),
        "status": "error",
    }
    if args.local_ai_strict_local and not is_local_endpoint(args.local_ai_endpoint):
        return {"_meta": meta,
                "_unavailable_reason": "endpoint non locale bloccato da "
                "--local-ai-strict-local (%s)" % args.local_ai_endpoint}
    try:
        context = build_local_ai_har_context(
            result, args.local_ai_max_entries, args.local_ai_raw)
        prompt = build_local_ai_prompt(context)
        raw = call_local_ai_ollama(
            args.local_ai_endpoint, args.local_ai_model, prompt, args.local_ai_timeout)
        validated = validate_local_ai_har_result(raw)
        meta["status"] = "ok"
        ...
        return {"_meta": meta,
                "_unavailable_reason": "connessione al modello locale fallita: %s" % reason}
    except (ValueError, json.JSONDecodeError) as exc:
        return {"_meta": meta,
                "_unavailable_reason": "risposta del modello non valida: %s" % exc}
    except Exception as exc:  # timeout, schema, ecc. — mai fatale per il report
        return {"_meta": meta,
                "_unavailable_reason": "%s: %s" % (type(exc).__name__, exc)}


def render_local_ai_html_section(ai_result: dict) -> str:
    """Render the 'AI Local RCA Review' HTML block (handles unavailable case)."""
    esc = html_module.escape
    note = ('<p class="dim" style="margin-top:12px;font-style:italic;">'
            'Nota: Analisi generata da modello locale. '
            'Usare come supporto, non come fonte unica.</p>')

    if not ai_result or ai_result.get("_unavailable_reason"):
        reason = esc(str(ai_result.get("_unavailable_reason", "motivo sconosciuto"))) \
            if ai_result else "non eseguita"
        return ('<details open><summary>AI Local RCA Review</summary>'
                '<div class="detail-content">'
                '<p style="color:#fd7e14;"><strong>Local AI Review non disponibile:</strong> '
                + reason + '</p>' + note + '</div></details>')
    ...
    def _ul(title, items):
        ...
        if not items:
            return ""
        lis = "".join("<li>%s</li>" % esc(str(i)) for i in items)
        return "<h4>%s</h4><ul>%s</ul>" % (esc(title), lis)

    parts = ['<details open><summary>AI Local RCA Review</summary>',
             '<div class="detail-content">']
    parts.append("<p><strong>Root cause probabile (AI):</strong> %s</p>"
                 % esc(d.get("root_cause_probabile", "")))
    parts.append("<p><strong>Confidenza:</strong> %s &nbsp;|&nbsp; "
                 "<strong>Rischio falso positivo:</strong> %s</p>"
                 % (esc(d.get("confidenza", "")), esc(d.get("rischio_falso_positivo", ""))))
    if d.get("motivazione"):
        parts.append("<p><strong>Motivazione:</strong> %s</p>" % esc(d["motivazione"]))
    if d.get("impatto_utente"):
        parts.append("<p><strong>Impatto utente:</strong> %s</p>" % esc(d["impatto_utente"]))
    parts.append(_ul("Cause alternative", d.get("cause_alternative")))
    parts.append(_ul("Evidenze forti", d.get("evidenze_forti")))
    parts.append(_ul("Evidenze deboli", d.get("evidenze_deboli")))
    parts.append(_ul("Evidenze mancanti", d.get("evidenze_mancanti")))
    parts.append(_ul("Contraddizioni o ambiguita", d.get("contraddizioni_o_ambiguita")))
    parts.append(_ul("Azioni immediate", d.get("azioni_immediate")))

    prio = d.get("priorita_remediation") or []
    if prio:
        rows = ["<h4>Priorita remediation</h4><ol>"]
        for p in prio:
            rows.append("<li><strong>%s</strong> (impatto %s, complessita %s): %s</li>" % (
                esc(str(p.get("azione", ""))), esc(str(p.get("impatto", ""))),
                esc(str(p.get("complessita", ""))), esc(str(p.get("motivazione", "")))))
        rows.append("</ol>")
        parts.append("".join(rows))

    if d.get("executive_summary"):
        parts.append("<p><strong>Executive summary:</strong> %s</p>"
                     % esc(d["executive_summary"]))
    parts.append(note)
    parts.append("</div></details>")
...
def main() -> int:
    try:
        ...
        parser = argparse.ArgumentParser(
            description="HARootCase — Enterprise-grade HAR analysis tool",
        ...
        # ── Local AI RCA Review (opzionale, offline, Ollama-compatible) ──
        ai = parser.add_argument_group("Local AI RCA Review (opzionale, offline)")
        ai.add_argument("--local-ai", action="store_true",
                        help="Abilita il layer AI locale (Ollama) come revisore RCA")
        ai.add_argument("--local-ai-endpoint", default=LOCAL_AI_DEFAULT_ENDPOINT,
                        help="Endpoint Ollama (default: %s)" % LOCAL_AI_DEFAULT_ENDPOINT)
        ai.add_argument("--local-ai-model", default=LOCAL_AI_DEFAULT_MODEL,
                        help="Modello locale (default: %s)" % LOCAL_AI_DEFAULT_MODEL)
        ai.add_argument("--local-ai-timeout", type=int, default=LOCAL_AI_DEFAULT_TIMEOUT,
                        help="Timeout in secondi (default: %d)" % LOCAL_AI_DEFAULT_TIMEOUT)
        ai.add_argument("--local-ai-max-entries", type=int,
                        default=LOCAL_AI_DEFAULT_MAX_ENTRIES,
                        help="Max entry HAR rilevanti inviate al modello (default: %d)"
                             % LOCAL_AI_DEFAULT_MAX_ENTRIES)
        ai.add_argument("--local-ai-output", default="auto",
                        help="Path JSON AI (default: auto -> <report>_local_ai.json)")
        ai.add_argument("--local-ai-strict-local", dest="local_ai_strict_local",
                        action="store_true", default=True,
                        help="Permetti solo endpoint locali/privati (default: attivo)")
        ai.add_argument("--local-ai-no-strict-local", dest="local_ai_strict_local",
                        action="store_false",
                        help="Disabilita il vincolo endpoint locale")
        ai.add_argument("--local-ai-append-report", dest="local_ai_append_report",
                        action="store_true", default=True,
                        help="Aggiungi la sezione AI al report HTML (default: attivo)")
        ai.add_argument("--local-ai-no-append-report", dest="local_ai_append_report",
                        action="store_false",
                        help="Non aggiungere la sezione AI al report HTML")
        ai.add_argument("--local-ai-raw", action="store_true", default=False,
                        help="Invia anche snippet brevi di body (default: disattivo)")
        ...
        # ── Banner ──
        print(BOLD("\n╔══════════════════════════════════════════════════════════════════════════════╗"))
        print(BOLD("║" + f"HARootCase — Enterprise-Grade Diagnostics v{__version__}".center(78) + "║"))
        print(BOLD("╚══════════════════════════════════════════════════════════════════════════════╝"))
        ...
        # ── Determine HTML output path early (needed for AI JSON naming) ──
        html_path = None
        if not args.no_html:
            if args.html:
                html_path = args.html
        ...
        # ── Local AI RCA Review (opzionale, mai fatale per il report) ──
        if args.local_ai:
            print(f"\n  {BOLD('Local AI RCA Review')}:")
            print(f"    Endpoint: {args.local_ai_endpoint}")
            print(f"    Modello:  {args.local_ai_model}")
            ai_result = run_local_ai_review(result, args)
            ai_json_path = resolve_local_ai_output(args, har_files, html_path)
            try:
                write_local_ai_json(ai_result, ai_json_path)
                ai_written = True
            except Exception as exc:
                ai_written = False
                print(f"    {YELLOW('Impossibile scrivere il JSON AI:')} {exc}")
            reason = ai_result.get("_unavailable_reason")
            if reason:
                print(f"    {YELLOW('Local AI Review non disponibile:')} {reason}")
            else:
                print(f"    {GREEN('Review AI completata.')}")
            if ai_written:
                print(f"    JSON AI: {os.path.abspath(ai_json_path)}")
            # Attach to result so the HTML report can include the AI section.
            if args.local_ai_append_report:
                result.local_ai = ai_result
